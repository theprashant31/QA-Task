...
import fs from 'fs';
test('All product test id api test', async ({ request }) => {
    const token = fs.readFileSync('token.txt', 'utf-8');
    const warehouseResponse = await
    request.get('https://easecommerce.in/api/v2/manage/warehouse/master/all-product-of-a-warehouse-test/v72HJpTzfh9ycFunsDR2Uw', {
    headers: {
