...
import fs from 'fs';
test('Empty bin api test', async ({ request }) => {
    const token = fs.readFileSync('token.txt', 'utf-8');
    const warehouseResponse = await
    request.get('https://easecommerce.in/api/v2/manage/warehouse/get-empty-bin-list?page=1&limit=10&offset=0&warehouseId=cjUjfB6M5qMWK72EHJ8EhC', {
    headers: {
