...
import fs from 'fs';
test('Inventory combo list api test', async ({ request }) => {
    const token = fs.readFileSync('token.txt', 'utf-8');
    const warehouseResponse = await
    request.get('https://easecommerce.in/api/v2/manage/warehouse/warehouse-inventory-combo-list?page=1&limit=10&offset=0', {
    headers: {
