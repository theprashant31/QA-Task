...
import fs from 'fs';
test('Empty rack list api test', async ({ request }) => {
    const token = fs.readFileSync('token.txt', 'utf-8');
    const warehouseResponse = await
    request.get('https://easecommerce.in/api/v2/manage/warehouse/get-empty-rack-list?page=1&limit=10&offset=0&warehouseId=v72HJpTzfh9ycFunsDR2Uw', {
    headers: {
