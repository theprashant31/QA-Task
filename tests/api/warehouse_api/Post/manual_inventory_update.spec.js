...
//skip reason: No sample data available for this API.

import { test, expect } from '@playwright/test';
import fs from 'fs';
test.skip('Manual Inventory update api test', async ({ request }) => {
    const token = fs.readFileSync('token.txt', 'utf-8');
    const warehouseResponse = await
    request.post('https://easecommerce.in/api/v2/manage/warehouse/manual/update-inventory/import', {
    headers: {
